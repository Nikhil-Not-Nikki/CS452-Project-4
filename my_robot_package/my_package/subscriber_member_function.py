    ...
    def listener_callback(self, msg):
        ...
        for i in range(len(ranges)):
            ...
            if ranges[i] > msg.range_min and ranges[i] < msg.range_max:
                ...
                #            if self.observation_grid[temp_y][j] > 0:
                #                self.observation_grid[temp_y][j] -= 2
                height = len(self.observation_grid)
                width = len(self.observation_grid[0]) if self.observation_grid else 0  # Ensure there's at least one row

                #print("Height of observation_grid:", height)
                #print("Width of observation_grid:", width)
                ...
                for j in range(len(self.observation_grid)):
                    for k in range(len(self.observation_grid[0])):
                        ...
                        index = j * 50 + k  # Calculate the correct index in the 1D array
                        one_dim_arr[index] = self.observation_grid[j][k]

                # Debug output to ensure data size is correct
                #print("Final one_dim_arr length:", len(one_dim_arr))
        ...
        if self.observation_counter > 2000:
            ...
            # Print the dimensions of observation_grid
            height = len(self.observation_grid)
            width = len(self.observation_grid[0]) if self.observation_grid else 0  # Ensure there's at least one row
